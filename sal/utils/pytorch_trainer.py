...
class NiceTrainer:
    def __init__(
        ...
        assert all(
            [isinstance(x, BaseEvent) for x in self.events]
        ), "All events must be instances of the BaseEvent!"
        self.computed_variables = (
            computed_variables if computed_variables is not None else {}
        )
        ...
        self._extra_var_info_string = (
            info_string if isinstance(info_string, str) else " - ".join(info_string)
        ) + (
            (" - " if self.printable_vars else "")
            + " - ".join("%s: {%s:.4f}" % (e, e) for e in self.printable_vars)
        )
    ...
    def _main_loop(self, is_training, steps=None, allow_switch_mode=True):
        ...
        smoothing_dict = dict(comp_time=None, data_time=None)
        smoothing_dict.update(
            dict(list(zip(self.printable_vars, len(self.printable_vars) * [None])))
        )
        ...
        for batch in dts_iter:
            ...
            smoothing_update = dict(
                comp_time=t_end - t_start, data_time=t_start - t_fetch
            )
            ...
            smoothing_update.update(
                {
                    e: to_number(self.pt_store[e])
                    for e in self.printable_vars
                    if e not in self._core_info_vars
                }
            )

            # perform smoother update
            smoothing_dict_update(
                smoothed_printable_vars, smoothing_update, smooth_coef
            )

            # calculate
            batches_per_sec = 1.0 / (
                smoothed_printable_vars["comp_time"]
                + smoothed_printable_vars["data_time"]
            )
            batches_done += 1
            steps_done_here += 1
            ...
            self.info_vars.update(
                dict(
                    ...
                    total_examples_done=self.info_vars["total_examples_done"]
                    + batch_size,
        ...
        if (
            steps is not None
        ):  # really annoying that pytorch does not handle this on its own... todo improve this!
    ...
    def validate(self, allow_switch_mode=False):
        ...
        print("Validation:")
        self._main_loop(
            is_training=False, steps=None, allow_switch_mode=allow_switch_mode
        )
    ...
    def _get_state(self):
        return dict(
            info_vars={
                k: v
                for k, v in list(self.info_vars.items())
                if k in self._core_info_vars
            },
    ...
    def _set_state(self, state):
        ...
        if len(self.modules) != len(state["state_dicts"]):
            raise ValueError(
                "The number of save dicts is different from number of models"
            )
    ...
    def save(self, step=1):
        ...
        torch.save(
            self._get_state(), os.path.join(self.save_dir, "model-%d.ckpt" % step)
        )
...
def auto_norm(im, auto_normalize=True, auto_fix=True):
    ...
    max_v = np.max(im)
    if (
        0 < np.max(im) - np.min(im) < 11.0 and auto_normalize
    ):  # something wrong, perform normalisation
    ...
    if n == 1:
        ...
        return np.mean(
            (
                np.sum(
                    np.cumsum(
                        np.argsort(probs, axis=1) == np.expand_dims(labels, 1), 1
                    ),
                    1,
                )
                <= n
...
def accuracy_calc_op(
    logits_from="logits", labels_from="labels", top_n=1, avg_preds_over=1
):
    """
