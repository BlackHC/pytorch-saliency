...
def ReducedExtractor(
    ...
    for layers in downsampling_blocks:
        ...
        _modules.append(
            ReducedCNNBlock(
                current_channels, base_channels, stride=2, activation_fn=activation_fn
            )
        )
        current_channels = base_channels
...
class UNetUpsampler(Module):
    def __init__(
        ...
        assert (
            follow_up_residual_blocks >= 1
        ), "You must follow up with residuals when using unet!"
...
class EasyModule(Module):
    ...
    def restore(self, save_dir, step=1):
        ...
        if not os.path.exists(p):
            print(
                WARN_TEMPLATE
                % ("Could not find any checkpoint at %s, skipping restore" % p)
            )
            return
...
def one_hot(labels, depth):
    if labels.is_cuda:
        return Variable(
            torch.zeros(labels.size(0), depth)
            .cuda()
            .scatter_(1, labels.long().view(-1, 1).data, 1)
        )
    else:
        return Variable(
            torch.zeros(labels.size(0), depth).scatter_(
                1, labels.long().view(-1, 1).data, 1
            )
        )
...
class Bottleneck(Module):
    def __init__(
        ...
        if stride != 1 or in_channels != out_channels:
            self.residual_transformer = Conv2d(
...
def SimpleGenerator(
    in_channels, base_channels, upsampling_blocks=lambda: torch.nn.ReLU(inplace=False)
):
    _modules = []
    current_channels = in_channels
    ...
    for layers in upsampling_blocks:
        if layers - 1 > 0:
            _modules.append(
                SimpleCNNBlock(current_channels, base_channels, layers=layers - 1)
            )
            current_channels = base_channels
...
class B4D_4D(Module):
    ...
    def __init__(
        self, in_channels, out_channels, preprocess_layers=2, keep_resolution=False
    ):
...
class GanGroup(Module):
    ...
    def generator_loss(self):
        return sum(
            self.calc_adv_loss(self._modules[k](i), 1.0)
            for k, i in zip(self.keys, self.fakes)
        ) / len(self.keys)
    ...
    def get_train_event(
        self, disc_steps=3, optimizer=torch_optim.SGD, **optimizer_kwargs
    ):
