...
def apply_mask(
    ...
    if random_colors:
        if cuda:
            alt += (
                torch.Tensor(n, c, 1, 1)
                .cuda()
                .uniform_(-color_range / 2.0, color_range / 2.0)
            )
        else:
            alt += torch.Tensor(n, c, 1, 1).uniform_(
                -color_range / 2.0, color_range / 2.0
            )

    alt = Variable(alt, requires_grad=False)

    if blurred_version_prob > 0.0:  # <- it can be a scalar between 0 and 1
        cand = gaussian_blur.gaussian_blur(
            images, kernel_size=blur_kernel_size, sigma=blur_sigma
        )
        if cuda:
            when = Variable(
                (
                    torch.Tensor(n, 1, 1, 1).cuda().uniform_(0.0, 1.0)
                    < blurred_version_prob
                ).float(),
                requires_grad=False,
            )
        else:
            when = Variable(
                (
                    torch.Tensor(n, 1, 1, 1).uniform_(0.0, 1.0) < blurred_version_prob
                ).float(),
                requires_grad=False,
            )
    ...
    im = Variable(
        torch.Tensor(
            np.expand_dims(
                np.transpose(
                    np.array(
                        Image.open(os.path.join(os.path.dirname(__file__), "test.jpg"))
                    ),
                    (2, 0, 1),
                ),
                0,
