...
def _gaussian_kernels(kernel_size, sigma, chans):
    ...
    x = np.expand_dims(
        np.array(list(range(-kernel_size // 2, -kernel_size // 2 + kernel_size, 1))), 0
    )
...
def gaussian_blur(_images, kernel_size=55, sigma=11):
    ...
    kernel_a, kernel_b = _gaussian_kernels(
        kernel_size=kernel_size, sigma=sigma, chans=_images.size(1)
    )
...
def test():
    ...
    assert (
        np.mean(gr) > 0.9
        and np.mean(np.flip(gr, 1) - gr) < 1e-6
