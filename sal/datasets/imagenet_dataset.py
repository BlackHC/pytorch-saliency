...
def get_train_dataset(size=224):
    if not (
        os.path.exists(IMAGE_NET_TRAIN_PATH) and os.path.exists(IMAGE_NET_VAL_PATH)
    ):
        raise ValueError(
...
def get_val_dataset(size=224):
    ...
    return ImageFolder(
        IMAGE_NET_VAL_PATH,
        transform=Compose([Scale(224), CenterCrop(size), ToTensor(), STD_NORMALIZE]),
    )
...
CLASS_ID_TO_NAME = {
    i: SYNSET_TO_NAME[CLASS_ID_TO_SYNSET[i]] for i in CLASS_ID_TO_SYNSET
}
