    ...
    if isinstance(x, np.ndarray):
        ...
        if c == required_rank:
            ...
            return to_batch_variable(
                torch.unsqueeze(torch.from_numpy(x), dim=0), required_rank, cuda
            )
        else:
            raise ValueError()
    if cuda:
...
def load_image_as_variable(path, cuda=False):
    ...
    return to_batch_variable(
        np.expand_dims(np.transpose(np.array(Image.open(path)), (2, 0, 1)), 0)
        / 255.0
        * 2
        - 1.0,
        4,
...
def test(cuda=True):
    ...
    ims = load_image_as_variable(
        os.path.join(os.path.dirname(__file__), "sal/utils/test2.jpg"), cuda=cuda
    )
    ...
    print(
        "You should expect ~200 images per second on a GPU (Titan XP) and 2.5 images per second on a CPU. "
    )
