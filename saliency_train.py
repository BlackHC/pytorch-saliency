...
optim_phase1 = torch_optim.Adam(
    saliency.selector_module.parameters(), 0.001, weight_decay=0.0001
)
optim_phase2 = torch_optim.Adam(
    saliency.get_trainable_parameters(), 0.001, weight_decay=0.0001
)
...
def ev_phase1(_images, _labels):
    ...
    _targets = (
        _labels
        + Variable(torch.Tensor(_images.size(0)).uniform_(1, 999).cuda()).long()
        * __fakes.long()
    ) % 1000
...
def ev_phase2(_images, _labels):
    ...
    _masks, _exists_logits, _ = saliency_p(_images, _targets)
    PT(exists_logits=_exists_logits)
    saliency_loss = saliency_loss_calc.get_loss(
        _images, _labels, _masks, _is_real_target=_is_real_label, pt_store=PT
    )
...
nt_phase1 = NiceTrainer(
    ...
    computed_variables={
        "exists_accuracy": accuracy_calc_op("exists_logits", "is_real_label")
    },
)
FAKE_PROB = 0.5
...
print(
    GREEN_STR
    % "Finished phase 1 of training, waiting until the dataloading workers shut down..."
)
...
saliency.minimalistic_save(
    "yoursaliencymodel"
)  # later to restore just use saliency.minimalistic_restore methdod.
