...
class RealTimeSaliency(wx.Frame):
    ...
    def update(self):
        self.info.SetLabel(
            "Showing: %s (logits: %f)" % (CLASS_ID_TO_NAME[TO_SHOW], LOGITS[TO_SHOW])
        )
    ...
    def choose_class(self, event):
        global TO_SHOW
        TO_SHOW = CLASS_NAME_TO_ID[
            self.list_ctrl.GetItem(self.list_ctrl.GetFocusedItem()).GetText()
        ]
...
class RT:
    ...
    def transform(self):
        while not self._stop:
            ...
            done_imgs = (
                (self.processor(imgs / (255.0 / 2) - 1.0) + 1) * (255.0 / 2.0)
            ).astype(np.uint8)
    ...
    def display(self):
        ...
        while not self._stop:
            ...
            self.delay = self.DELAY_SMOOTH * self.delay + (1.0 - self.DELAY_SMOOTH) * (
                time.time() - creation_time
            )
            while time.time() < creation_time + self.delay:
                time.sleep(POLL_DELAY)
            self.time_per_frame = 0.9 * self.time_per_frame + 0.1 * (
...
def get_proc_fn(cuda=False):
    ...
    def proc(ims):
        ...
        if FAST_MODE:
            ...
            sq = cv2.resize(
                np.squeeze(ims, 0), (288, 512), interpolation=cv2.INTER_AREA
            )
...
def get_probs_np_img(probs, num=6):
    ...
    objects = [
        "\n".join(textwrap.wrap(imagenet_dataset.CLASS_ID_TO_NAME[e], 15)[:2])
        for e in top_k
    ]
